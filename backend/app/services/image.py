...
async def process_and_upload_with_bytes(
    image_base64: Optional[str] = None,
    image_url: Optional[str] = None,
) -> tuple[str, bytes]:
    """
    Like process_and_upload but also returns the WebP bytes.
    Use this when you need the bytes for in-memory embedding before they're gone.
    Returns (r2_url, webp_bytes).
    """
    if not r2_configured() and image_url and not image_base64:
        # Pass-through mode: fetch bytes for embedding even though we won't re-upload
    ...
    img = Image.open(io.BytesIO(raw))
    webp_bytes = _to_webp_bytes(img)
    r2_url = await upload_image_bytes(webp_bytes, "image/webp")
    return r2_url, webp_bytes
